...
# Récupération des mesures dans les fichiers
mois=[]
poids=[]
taille=[]
crane=[]
...
# Import des courbes de référence depuis les different fichiers en fonction du genre
path='/home/bnp-renault-014/Code/python/Projet/constantes-nourrissons/'
# if genre=="f":
#     with open(f'{path}poids-age-fille-0-60.csv','r') as csv_poids_fille_file:
#         try:
#             read_poids_fille = csv.reader(csv_poids_fille_file, delimiter=';')
#             headers = next(read_poids_fille)
#             for row in read_poids_fille:
#                 P5.append(float(row[7]))
#                 P25.append(float(row[10]))
#                 P50.append(float(row[11]))
#                 P75.append(float(row[12]))
#                 P95.append(float(row[15]))
#         finally:
#             csv_poids_fille_file.close()
...
def charger_fichier(gender):
    ...
    with open(f'{path}taille-age-{name_gender}-0-60.csv','r') as csv_file:
        try:
            ...
            headers = next(reader)
            for row in reader:
                d5_tailles.append(float(row[7]))
                d25_tailles.append(float(row[10]))
                d50_tailles.append(float(row[11]))
                d75_tailles.append(float(row[12]))
                d95_tailles.append(float(row[15]))
        finally:
            csv_file.close()
    with open(f'{path}perim-cra-age-{name_gender}-0-60.csv','r') as csv_file:
        try:
            ...
            headers = next(reader)
            for row in reader:
                d5_cranes.append(float(row[7]))
                d25_cranes.append(float(row[10]))
                d50_cranes.append(float(row[11]))
                d75_cranes.append(float(row[12]))
                d95_cranes.append(float(row[15]))
        finally:
            csv_file.close()      
