...
while True:
    try:
        age = int(input('Entrez l\'age de votre nourisson en mois (entre 0 et 60 mois): '))
        if (age>=0 and age <=60):
            break
        if age <0 or age >60:
            print('Svp entrer un nombre entre 0 et 60, pas plus pas moins !')
            continue
    except ValueError:
        print('Svp entrer un nombre entier')

while True:
    try:
        poids = float(input('Veuillez entrer le poids de votre nourrisson en kg : '))
        if (poids>0):
            break
        if (poids<0):
            print('Svp entrez un nombre positif !')
            continue    
    except ValueError:
        print('Svp entrez une valeur numérique')

while True:
    try:
        taille = float(input('Veuillez entrer la taille de votre nourrisson en cm : '))
        if taille>0:
            break
        if taille<=0:
            print('Svp entrez un nombre positif !')
...
while True:
    try:
        perimetre = float(input('Veuillez entrer le périmètre cranien de votre nourrisson en cm : '))
        if perimetre>0 :
            break
        if perimetre<=0:
            print('Svp entrez un nombre positif !')
...
def check_weight_in_norm(gender,mass,month):
    if gender == "f":
        in_norm=((low_weights_girls[age]<mass) and (high_weights_girls[age]>mass))
        print(f'La norme de poids pour une fille de {month} mois est située entre {low_weights_girls[age]} et {high_weights_girls[age]}')
        if in_norm:
            print(f'le poids de votre nourrisson ({mass}) est dans la norme.')
        else:
            print(f'le poids de votre nourrisson ({mass}) n\'est pas dans la norme.')
    if gender == "g":
        in_norm=((low_weights_boys[age]<mass) and (high_weights_boys[age]>mass))
        print(f'La norme de poids pour un garçon de {month} est située entre {low_weights_boys[age]} et {high_weights_boys[age]}')
        if in_norm:
            print(f'le poids de votre nourrisson ({mass}) est dans la norme.')
        else:
            print(f'le poids de votre nourrisson ({mass}) n\'est pas dans la norme.')
...
def check_height_in_norm(gender,heigh,month):
    if gender == "f":
        in_norm=((low_heights_girls[age]<heigh) and (high_heights_girls[age]>heigh))
        print(f'La norme de taille pour une fille de {month} mois est située entre {low_heights_girls[age]} et {high_heights_girls[age]}')
        if in_norm:
            print(f'la taille de votre nourrisson ({heigh}) est dans la norme.')
        else:
            print(f'la taille de votre nourrisson ({heigh}) n\'est pas dans la norme.')
    if gender == "g":
        in_norm=((low_heights_boys[age]<heigh) and (high_heights_boys[age]>heigh))
        print(f'La norme de taille pour un garçon de {month} est située entre {low_heights_boys[age]} et {high_heights_boys[age]}')
        if in_norm:
            print(f'la taille de votre nourrisson ({heigh}) est dans la norme.')
        else:
            print(f'la taille de votre nourrisson ({heigh}) n\'est pas dans la norme.')
...
def check_skull_in_norm(gender,skull,month):
    if gender == "f":
        in_norm=((low_skulls_girls[age]<skull) and (high_skulls_girls[age]>skull))
        print(f'La norme de périmètre cranien pour une fille de {month} mois est située entre {low_skulls_girls[age]} et {high_skulls_girls[age]}')
        if in_norm:
            print(f'Le périmètre cranien de votre nourrisson ({skull}) est dans la norme.')
        else:
            print(f'Le périmètre cranien de votre nourrisson ({skull}) n\'est pas dans la norme.')
    if gender == "g":
        in_norm=((low_skulls_boys[age]<skull) and (high_skulls_boys[age]>skull))
        print(f'La norme de périmètre cranien pour un garçon de {month} est située entre {low_skulls_boys[age]} et {high_skulls_boys[age]}')
        if in_norm:
            print(f'Le périmètre cranien de votre nourrisson ({skull}) est dans la norme.')
        else:
            print(f'Le périmètre cranien de votre nourrisson ({skull}) n\'est pas dans la norme.')
...
check_weight_in_norm(genre,poids,age)
check_height_in_norm(genre,taille,age)
check_skull_in_norm(genre,perimetre,age)

# def check_taille_in_norm(genre) :
#     return

# def check_perimeter_in_norm:
#     return

# def display_weigh_status(gender,mass,month):
#     if check_weight_in_norm(genre,poids,age)[0]:
        
#     return

# def display_height_status:
#     return

# def display_perimeter:
#     return
